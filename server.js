...
const pool = new Pool({
  ...
  ssl: {
    rejectUnauthorized: false, // Neon DB için gerekli
  },
});
...
app.get('/api/students', async (req, res) => {
  ...
  try {
    ...
    // Admin hepsini görür, WHERE eklenmez.
  ...
  try {
    ...
    if (role === 'parent') {
      ...
      params = [userId];
       ...
       // Tek bir öğrencinin tüm raporları (Bütünleşik rapor için)
       query += ' WHERE s.student_id = $1';
       params = [userId]; // userId burada studentId olarak kullanılıyor
    }
    ...
    // Veritabanından gelen snake_case alanları frontend'in beklediği camelCase'e çeviriyoruz
    const formattedRows = result.rows.map(row => ({
      ...
      date: row.date,
      categoryScores: row.category_scores, // JSONB otomatik parse edilir
      aiAnalysis: row.ai_analysis // JSONB otomatik parse edilir
    }));
...
app.post('/api/screenings', async (req, res) => {
  ...
  try {
    ...
    // Frontend formatına uygun dönüş
...
app.get('/api/messages', async (req, res) => {
  ...
  try {
    // Hem gönderilen hem alınan mesajları getir ve gönderen ismini ekle
    const query = `
...
app.post('/api/messages', async (req, res) => {
  ...
  try {
    // Gerçek bir senaryoda receiverId, kullanıcının bağlı olduğu öğretmen/veli ID'si olmalı.
    // Demo kolaylığı için eğer receiverId 'demo...' ise veritabanından gerçek bir ID bulmaya çalışabiliriz.
    // Ancak şimdilik client'ın doğru ID gönderdiğini varsayıyoruz.
